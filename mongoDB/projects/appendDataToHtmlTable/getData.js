DATA_TABLE 			= [];											// Holds the data from the queries.
connections		 	= [];											// connections[] = Connected sockets.
...
io.sockets.on('connection', function (socket) {						// Connection made.
    connections.push(socket);
    console.log('Connected: %s sockets connected', connections.length);

	socket.on('get data', getData);

    socket.on('disconnect', function (data) {						// Disconnected
   ...
   function getData() {
		MongoClient.connect(url, function (err, db) {			// Use connect method to connect to the mongodb server.
			if (err) {
				...
				getAllData(db);
				io.sockets.emit('set data', DATA_TABLE);
